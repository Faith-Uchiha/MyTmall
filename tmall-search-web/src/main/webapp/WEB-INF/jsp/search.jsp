    ...
    <div class="r-filter">
      <div class="f-sort">
        <div class="pagin">
          ...
          <a class="prev" href="http://localhost:8085/search.html?keyword=${query}&page=<c:if test="${page==1}">1</c:if><c:if test="${page>1}">${page-1}</c:if>">
          上一页</a>
          <a class="next" href="http://localhost:8085/search.html?keyword=${query}&page=<c:if test="${page==totalPages}">${totalPages }</c:if><c:if test="${page<totalPages}">${page+1}</c:if>">下一页  </a>    	
         </div>
